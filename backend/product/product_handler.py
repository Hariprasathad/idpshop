...
def lambda_handler(event, context):
    try:
        ...
        if query:
            ...
            # Paginate search results in-memory
            ...
            if last_key:
                try:
                    start_idx = next(i for i, p in enumerate(products) if p["productId"] == last_key) + 1
                except StopIteration:
                    start_idx = 0

            paginated_products = products[start_idx : start_idx + limit]
            next_key = None
            if start_idx + limit < len(products):
                next_key = paginated_products[-1]["productId"]

            return response(200, {
                "products":
                convert_decimal(paginated_products),
                "count":
                len(paginated_products),
                "lastKey":
                next_key
            })
